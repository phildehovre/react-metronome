...
export default function Tempo({ bpm, setBpm, startClick, tapTempo, light, isPlaying, soundEffect, setSoundEffect }) {
...
const renderedSoundEffects = soundEffects.map(effect => {
    return (
        <option value={effect}>{`${effect[0].toUpperCase()}${effect.slice(1)}`}</option>
    )
})
...
return (
    ...
    <div >
            ...
            <div className="ui header" style={injectSpan}>
                ...
                <button className="ui button" onClick={e => setBpm(bpm + 1)} >+</button>
                <button className="ui button" onClick={startClick}>
                    <i className="play icon"></i>
                </button>
